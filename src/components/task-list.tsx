    ...
    <div className="w-full max-w-3xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" className="text-sm">List</Button>
          <Button variant="outline" className="text-sm">Board</Button>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        <TaskSection title="In-Progress (3)" status="IN_PROGRESS" tasks={tasks} />
...
function TaskSection({ title, status, tasks }: { title: string; status: TaskStatus; tasks: Task[] }) {
  const filteredTasks = tasks.filter(task => task.status === status)
  const bgColor = status === 'TODO' ? 'bg-pink-50' :
